...
export function DiscordPanel({forms}:{forms:FormDefinition[]}) {
 ...
 async function save(e:React.FormEvent){e.preventDefault();setBusy('save');setError('');setNotice('');try{await request('POST',{action:'save',url:webhook.trim()});setWebhook('');setConfigured(true);setNotice('Webhook saved.');setSent('');}catch(e){setError((e as Error).message);}finally{setBusy('');}}

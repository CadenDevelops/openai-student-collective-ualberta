...
/* Discord's own caps, from the message resource docs. maxLength stops typing
   at the limit, which is silent on its own, so every capped control shows what
   the limit is rather than only refusing the next keystroke. */
const limits={content:2000,title:256,description:4096,fieldName:256,fieldValue:1024,footer:2048,authorName:256,url:2048,fields:25,total:6000};

function Count({n,max}:{n:number;max:number}) {
 return <span className={`field-count${n>=max?' at-limit':''}`}>{n.toLocaleString()} / {max.toLocaleString()}</span>;
}
function Input({label,value,onChange,max=limits.url,type='text',placeholder,counter}:{label:string;value:string;onChange:(v:string)=>void;max?:number;type?:string;placeholder?:string;counter?:boolean}) {
 return <label>{label}<input type={type} value={value} onChange={e=>onChange(e.target.value)} maxLength={max} placeholder={placeholder}/>{counter&&<Count n={value.length} max={max}/>}</label>;
}
 ...
 return <div className="discord-workspace">
  ...
  <div className="discord-compose-grid">
   <form className="discord-composer" onSubmit={send}><fieldset disabled={!!busy||loading} className="editor-lock">
    ...
    {draft.mode==='message'?<><label>Message<textarea rows={10} value={draft.content} maxLength={limits.content} placeholder="What would you like to share?" onChange={ev=>{setDraft(d=>({...d,content:ev.target.value}));setNotice('');}}/></label><div className="discord-character-count"><Count n={draft.content.length} max={limits.content}/></div></>:<>
     <Input label="Title" value={e.title} onChange={v=>patch({title:v})} max={limits.title} counter placeholder="Event name or announcement"/>
     <label>Description<textarea rows={6} value={e.description} maxLength={limits.description} onChange={ev=>patch({description:ev.target.value})} placeholder="Add the details."/><Count n={e.description.length} max={limits.description}/></label>
     ...
     <details className="embed-options"><summary>Author</summary><Input label="Author name" value={e.authorName} onChange={v=>patch({authorName:v})} max={limits.authorName} counter/><Input label="Author link" type="url" value={e.authorUrl} onChange={v=>patch({authorUrl:v})}/><Input label="Author icon URL" type="url" value={e.authorIcon} onChange={v=>patch({authorIcon:v})}/></details>
     ...
     <details className="embed-options"><summary>Fields <span>{e.fields.length} / {limits.fields}</span></summary>{e.fields.map((f,i)=><div className="embed-field-editor" key={i}><Input label={`Field ${i+1} name`} value={f.name} max={limits.fieldName} counter onChange={v=>patch({fields:e.fields.map((x,n)=>n===i?{...x,name:v}:x)})}/><label>Field {i+1} value<textarea rows={2} maxLength={limits.fieldValue} value={f.value} onChange={ev=>patch({fields:e.fields.map((x,n)=>n===i?{...x,value:ev.target.value}:x)})}/><Count n={f.value.length} max={limits.fieldValue}/></label><div className="embed-field-actions"><label className="inline-check"><input type="checkbox" checked={f.inline} onChange={ev=>patch({fields:e.fields.map((x,n)=>n===i?{...x,inline:ev.target.checked}:x)})}/>Inline</label><div className="actions"><button type="button" aria-label={`Move field ${i+1} up`} disabled={i===0} onClick={()=>{const fields=[...e.fields];[fields[i-1],fields[i]]=[fields[i],fields[i-1]];patch({fields});}}>↑</button><button type="button" aria-label={`Move field ${i+1} down`} disabled={i===e.fields.length-1} onClick={()=>{const fields=[...e.fields];[fields[i+1],fields[i]]=[fields[i],fields[i+1]];patch({fields});}}>↓</button><button type="button" onClick={()=>patch({fields:e.fields.filter((_,n)=>n!==i)})} aria-label={`Remove field ${i+1}`}>Remove</button></div></div></div>)}<button type="button" className="action" disabled={e.fields.length>=limits.fields} onClick={()=>patch({fields:[...e.fields,{name:'',value:'',inline:false}]})}>Add field</button></details>
     <details className="embed-options"><summary>Footer & timestamp</summary><Input label="Footer text" value={e.footer} max={limits.footer} counter onChange={v=>patch({footer:v})}/><Input label="Footer icon URL" type="url" value={e.footerIcon} onChange={v=>patch({footerIcon:v})}/><label className="inline-check"><input type="checkbox" checked={e.timestamp} onChange={ev=>patch({timestamp:ev.target.checked})}/>Include send time</label></details>
     <div className={`discord-character-count embed-total${total>limits.total?' at-limit':''}`}>{total.toLocaleString()} / {limits.total.toLocaleString()} total characters{total>limits.total?'. Discord counts title, description, author, footer and every field together, and refuses the message over this.':''}</div>
    </>}
    {hasContent&&validation&&<p className="small muted" role="status">{validation}</p>}
    <details className="discord-limits"><summary>Limits and restrictions</summary><ul>
     <li>Messages post as <strong>{DISCORD_NAME}</strong> with the collective’s logo, whatever the channel’s webhook is named.</li>
     <li><strong>@everyone, @here and role pings never fire.</strong> They appear as plain text, so a mention cannot be sent by accident.</li>
     <li>A plain message holds {limits.content.toLocaleString()} characters. An embed holds {limits.description.toLocaleString()} in its description, but title, description, author, footer and all field text together must stay under {limits.total.toLocaleString()}.</li>
     <li>Up to {limits.fields} fields, each needing both a name and a value.</li>
     <li>An author link or icon needs an author name, and a footer icon needs footer text.</li>
     <li>Every link must be http or https, and image URLs must point straight at an image file.</li>
     <li>Discord accepts a message once. Editing after sending creates a new message rather than changing the old one.</li>
    </ul></details>
